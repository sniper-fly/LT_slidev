...
- `readability`: フォントサイズ最小値、行間、本文密度。
   **カード・ボックス・表セル・キャプション等、本文以外の領域に置かれた文字も例外なく最小フォントサイズ基準の対象**とする
   (「装飾要素の中だから小さくてよい」という抜け道を作らない。カード内テキストが本文より小さい `text-xl` 以下になっていないか個別に確認する)
...
- `wording`: 見出し・箇条書きの**文字量としての冗長性** (長さ・重複による伝達効率の問題)。
   ...
   **図解パターン (並列カード・矢印フロー・Before/After 等) の上に、スライドタイトルや図の内容とほぼ同じ主張を言い換えただけの要約見出しが追加されていないか**も対象とする
...
- 本文・注釈・コードの最小フォントが可読範囲 (本文 ≥ 24px / 見出し ≥ 40px が目安。
   主役のコマンド・数式・短い結論はさらに大きく)。**この基準はカード・ボックス・表セル内の文字にも例外なく適用する**
...
- 見出しは本文の要約になっており、本文と重複せず、削っても意味が変わらない修飾語が残っていない。
   図解パターン (並列カード・矢印フロー等) を使っている場合、スライドタイトルや図の内容の言い換えに過ぎない要約見出しが別途追加されていない
